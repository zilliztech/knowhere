...
GetKey(const std::string& name) {
    ...
    } else if (std::is_same_v<DataType, sparse_u32_f32>) {
        return name + std::string("_sparse_u32_f32");
    }
}
...
data_type_conversion(const DataSet& src, const std::optional<int64_t> start = std::nullopt,
    ...
    // for emb_list
    auto lims = src.Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET);
    if (lims != nullptr) {
        size_t lims_size = 0;
        ...
        des->Set(knowhere::meta::EMB_LIST_OFFSET, lims_data.release());
    }
    return des;
}
