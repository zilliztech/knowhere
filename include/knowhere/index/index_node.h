 ...
 */
    ...
    SetInternalIdToMostExternalIdMap(std::vector<uint32_t>&& map) {
        internal_id_to_most_external_id = std::move(map);
    ...
    SetBaseIndexIDMap() {
        ...
        std::vector<uint32_t> internal_id_to_most_internal_id_map(id_map_size);
        for (size_t i = 0; i < id_map_size; i++) {
            internal_id_to_most_internal_id_map[i] = emb_list_offset_->get_el_id(internal_id_to_external_id_map->at(i));
        }
        return SetInternalIdToMostExternalIdMap(std::move(internal_id_to_most_internal_id_map));
    }
    ...
    BuildEmbListIfNeed(const DataSetPtr dataset, std::shared_ptr<Config> cfg, bool use_knowhere_build_pool = true) {
        ...
        auto lims = dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET);
        if (lims == nullptr) {
     ...
     */
    virtual Status
    DeserializeEmbListIfNeed(const BinarySet& binset, std::shared_ptr<Config> config) final {
    ...
    virtual Status
    DeserializeFromFileIfNeed(const std::string& filename, std::shared_ptr<Config> config) final {
        auto cfg = static_cast<const knowhere::BaseConfig&>(*config);
        auto el_metric_type_or = get_el_metric_type(cfg.metric_type.value());
        if (!el_metric_type_or.has_value()) {
            // if not emb_list, use the default deserialize method
        ...
        try {
            auto emb_list_meta_file_name = cfg.emb_list_meta_file_name.value();
            if (emb_list_meta_file_name.empty()) {
                LOG_KNOWHERE_WARNING_ << "emb_list_meta_file is empty, but metric type is emb_list";
                return Status::emb_list_inner_error;
            }

            std::ifstream emb_list_meta_file(emb_list_meta_file_name, std::ios::binary);
            if (!emb_list_meta_file.is_open()) {
                LOG_KNOWHERE_WARNING_ << "emb_list_meta_file does not exist: " << emb_list_meta_file_name;
                return Status::emb_list_inner_error;
            }
            size_t size = 0;
    ...
    SearchEmbListIfNeed(const DataSetPtr dataset, std::unique_ptr<Config> config, const BitsetView& bitset,
        ...
        bool query_is_emb_list = dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET) != nullptr;
    ...
    SearchEmbList(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset,
        ...
        const size_t* lims = dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET);
        if (lims == nullptr) {
        ...
        for (size_t i = 0; i < num_q_el; i++) {
            ...
            for (const auto& el_id : el_ids_set) {
                ...
                if (!bf_search_res.has_value()) {
                    LOG_KNOWHERE_ERROR_ << "bf search error";
 ...
 protected:
    ...
    std::vector<uint32_t>
        internal_id_to_most_external_id;  // internal id to most external id, for 1-hop bitset check if emb_list is used
};
