...
TEST_CASE("Test Build Search Concurrency", "[Concurrency]") {
    ...
    SECTION("Test Build & Search Correctness") {
        ...
        auto ivf_flat =
            knowhere::IndexFactory::Instance().Create<float>(knowhere::IndexEnum::INDEX_FAISS_IVFFLAT, version);
        auto ivf_flat_cc =
            knowhere::IndexFactory::Instance().Create<float>(knowhere::IndexEnum::INDEX_FAISS_IVFFLAT_CC, version);
