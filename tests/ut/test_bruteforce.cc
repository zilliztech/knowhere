...
check_search_with_out_ids(const uint64_t nb, const uint64_t nq, const uint64_t dim, const int64_t k,
    ...
    for (auto i = 0; i < nq * k; i++) {
        ...
        // the difference cause by whether to choose  4 vectors or 1 vectors to get difference
#if defined(__ARM_NEON)
        REQUIRE(std::abs(gt_dis[i] - dis[i]) < 0.05);
#else
        REQUIRE(std::abs(gt_dis[i] - dis[i]) < 0.00001);
#endif
    }
}
