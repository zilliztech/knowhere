...
TEST_CASE("Brute Force preserves trailing empty embedding lists", "[emb_list][trailing_empty]") {
    ...
    std::array<size_t, 4> query_lims = {0, 1, 1, 1};
    ...
    query->Set(knowhere::meta::EMB_LIST_COUNT, int64_t{3});
    query->Set(knowhere::meta::NQ, int64_t{99});
    ...
    REQUIRE(result.value()->GetRows() == 3);
