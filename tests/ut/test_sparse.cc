...
TEST_CASE("Test Mem Sparse Index With Float Vector", "[float metrics]") {
    ...
    SECTION("Test Search") {
        ...
        {
            auto idx = knowhere::IndexFactory::Instance().Create<knowhere::sparse_fp32>(name, version).value();
            ...
            REQUIRE(idx.HasRawData(metric) ==
                    knowhere::IndexStaticFaced<knowhere::sparse_fp32>::HasRawData(name, version, json));
...
TEST_CASE("Test Mem Sparse Index CC", "[float metrics]") {
    ...
    SECTION("Test GetVectorByIds") {
        ...
        REQUIRE(idx.HasRawData(metric) == knowhere::IndexStaticFaced<knowhere::sparse_fp32>::HasRawData(name, version, json));
