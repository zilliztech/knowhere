...
fp16_vec_L2sqr_sve(const knowhere::fp16* x, const knowhere::fp16* y, size_t d) {
    ...
    while (i < d) {
        ...
        svfloat16_t diff_fp16 = svsub_f16_m(pg_16, a_fp16, b_fp16);
        svfloat32_t diff_fp32_low = svcvt_f32_f16_z(pg_32, svtrn1_f16(diff_fp16, diff_fp16));
        svfloat32_t diff_fp32_high = svcvt_f32_f16_z(pg_32, svtrn2_f16(diff_fp16, diff_fp16));
        sum1 = svmla_f32_m(pg_32, sum1, diff_fp32_low, diff_fp32_low);
        sum2 = svmla_f32_m(pg_32, sum2, diff_fp32_high, diff_fp32_high);

        i += svcnth();
    }
    float result = svaddv_f32(svptrue_b32(), sum1) + svaddv_f32(svptrue_b32(), sum2);
    return result;
}
...
fp16_vec_norm_L2sqr_sve(const knowhere::fp16* x, size_t d) {
    ...
    while (i < d) {
        ...
        svfloat16_t a_fp16 = svld1_f16(pg_16, reinterpret_cast<const __fp16*>(x + i));
        svfloat16_t square_fp16 = svmul_f16_m(pg_16, a_fp16, a_fp16);
        svfloat32_t square_fp32_low = svcvt_f32_f16_z(pg_32, svtrn1_f16(square_fp16, square_fp16));
        svfloat32_t square_fp32_high = svcvt_f32_f16_z(pg_32, svtrn2_f16(square_fp16, square_fp16));
