...
BruteForce::SearchWithBuf(const DataSetPtr base_dataset, const DataSetPtr query_dataset, int64_t* ids, float* dis,
    ...
    bool is_emb_list = base_dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET) != nullptr;
    ...
    auto nq = query_dataset->GetRows();
    bool query_is_emb_list = query_dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET) != nullptr;
    ...
    if (is_emb_list) {
        ...
        auto base_el_offset = EmbListOffset(base_dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET), nb);
        auto query_el_offset = EmbListOffset(query_dataset->Get<size_t*>(knowhere::meta::EMB_LIST_OFFSET), nq);
