 ...
 */
Status
minhash_lsh_hit_with_topk1_opt_search(const char* x, const char* y, size_t size_in_bytes, size_t element_size_in_bytes,
                                      size_t mh_lsh_band, size_t mh_lsh_r, size_t nx, size_t ny, size_t topk,
 ...
 */
Status
minhash_ny_batch_search(const char* x, const char* y, size_t size_in_bytes, size_t element_size_in_bytes,
                        size_t mh_lsh_band, size_t mh_lsh_r, size_t nx, size_t ny, size_t topk,
                        bool mh_search_with_jaccard, const BitsetView& bitset, float* distances, int64_t* labels) {
    auto pool = ThreadPool::GetGlobalSearchThreadPool();
...
OptimizeMinHashLSHParams(size_t original_dim, size_t band) {
    if (band <= 0) {
        size_t b = 1;
        return {b, original_dim};
    } else if (band > original_dim) {
        size_t b = original_dim;
        return {b, 1};
    } else {
        size_t new_dim = (original_dim / band) * band;
        size_t b = band;
        size_t r = new_dim / b;
        return {b, r};
    }
...
GenHashKV(const char* data, size_t rows, size_t data_size, size_t data_element_size, size_t band_num,
          size_t band_size) {
    auto res_kv = std::shared_ptr<minhash::KVPair[]>(new minhash::KVPair[band_num * rows]);
...
MinHashVecSearch(const char* x, const char* y, size_t size_in_bytes, size_t element_size_in_bytes, size_t mh_lsh_band,
    ...
    if (topk == 1 && !mh_search_with_jaccard) {  // most common case : topk = 1 and mh_search_with_jaccard == false
        return minhash_lsh_hit_with_topk1_opt_search(x, y, size_in_bytes, element_size_in_bytes, mh_lsh_band, mh_lsh_r,
                                                     nx, ny, topk, bitset, distances, labels);
    } else {
        return minhash_ny_batch_search(x, y, size_in_bytes, element_size_in_bytes, mh_lsh_band, mh_lsh_r, nx, ny, topk,
