...
struct MinHashLSHBuildParams {
    ...
    size_t block_size;
    bool with_raw_data;
    size_t mh_vec_element_size;
    size_t mh_vec_length;
};
    ...
    WarmUp() {
        if (mmap_enable_) {
            if (madvise(data_, block_size_ * blocks_num_, MADV_WILLNEED) == -1) {
                LOG_KNOWHERE_ERROR_ << "Failed to warmup band data : " << strerror(errno);
    ...
    CoolDown() {
        if (mmap_enable_) {
            if (madvise(data_, block_size_ * blocks_num_, MADV_DONTNEED) == -1) {
                LOG_KNOWHERE_ERROR_ << "Failed to cooldown band data : " << strerror(errno);
                return Status::disk_file_error;
            }
        }
        return Status::success;
...
class MinHashLSH {
 public:
    ...
    Status
    Search(const char* query, float* distances, idx_t* labels, MinHashLSHSearchParams* params);
    Status
 ...
 private:
    ...
    size_t mh_vec_elememt_size_;
    size_t mh_vec_length_;
    size_t ntotal_;
};
...
void
MinHashBandIndex::Search(KeyType key, MinHashLSHResultHandler* res, faiss::IDSelector* id_selector) {
    auto block_id = faiss::u64_binary_search_ge(maxs_.data(), maxs_.size(), key);

    if (block_id == -1 || key < mins_[block_id]) {
        return;
    }
    while (block_id < mins_.size() && key >= mins_[block_id]) {
        ...
        auto inner_id = faiss::u64_binary_search_eq(blk_k, rows, key);
        if (inner_id != -1) {
            for (; key == blk_k[inner_id] && inner_id < rows; inner_id++) {
...
MinHashLSH::BuildAndSave(MinHashLSHBuildParams* params) {
    ...
    // load raw data, generate hash kv for each band and save raw data
    {
        ...
        load_binary_vec(params->data_path, data, ntotal, bin_vec_dim);
        ...
        if (params->with_raw_data) {
            ...
            for (auto i = 0; i < ntotal; i += vec_num_a_blk) {
...
Status
MinHashLSH::Search(const char* query, float* distances, idx_t* labels, MinHashLSHSearchParams* params) {
    if (params == nullptr) {
...
MinHashLSH::BatchSearch(const char* query, size_t nq, float* distances, idx_t* labels, std::shared_ptr<ThreadPool> pool,
    ...
    while (access_list.size() && band_ofs < this->band_) {
        ...
        auto run_time = (access_num + kQueryBatch - 1) / kQueryBatch;
