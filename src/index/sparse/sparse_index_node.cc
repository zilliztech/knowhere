...
using label_t = sparse::label_t;

class SparseInvertedIndexNode : public IndexNode {
 public:
    explicit SparseInvertedIndexNode(const int32_t& /*version*/, const Object& /*object*/, bool use_wand)
        : index_(nullptr), search_pool_(ThreadPool::GetGlobalSearchThreadPool()), use_wand_(use_wand) {
    }
    ...
    Train(const DataSet& dataset, const Config& config) override {
        ...
        auto index = new sparse::InvertedIndex<float>();
        index->SetUseWand(use_wand_);
        index->SetDropRatioBuild(drop_ratio_build);
        if (index_ != nullptr) {
    ...
    Add(const DataSet& dataset, const Config& config) override {
        ...
        return index_->Add(dataset.GetTensor());
    }
    ...
    GetVectorByIds(const DataSet& dataset) const override {
        throw std::runtime_error("GetVectorByIds not supported for current index type");
    }
    ...
    Deserialize(const BinarySet& binset, const Config& config) override {
        ...
        index_ = new sparse::InvertedIndex<float>();
    ...
    Type() const override {
        return use_wand_ ? knowhere::IndexEnum::INDEX_SPARSE_WAND : knowhere::IndexEnum::INDEX_SPARSE_INVERTED_INDEX;
    }

 private:
    sparse::InvertedIndex<float>* index_;
    std::shared_ptr<ThreadPool> search_pool_;
    const bool use_wand_;
};  // class SparseInvertedIndexNode

KNOWHERE_REGISTER_GLOBAL(SPARSE_INVERTED_INDEX, [](const int32_t& version, const Object& object) {
    return Index<SparseInvertedIndexNode>::Create(version, object, /* use_wand = */ false);
});

KNOWHERE_REGISTER_GLOBAL(SPARSE_WAND, [](const int32_t& version, const Object& object) {
    return Index<SparseInvertedIndexNode>::Create(version, object, /* use_wand = */ true);
});
