...
class SparseDspIndexNode : public IndexNode {
 ...
 protected:
    ...
    CreateDspIndex(const SparseDspConfig& cfg) const {
        if (IsMetricType(cfg.metric_type.value(), metric::BM25)) {
            if (!cfg.bm25_k1.has_value() || !cfg.bm25_b.has_value() || !cfg.bm25_avgdl.has_value()) {
                return expected<sparse::DspIndexBase<value_type>*>::Err(Status::invalid_args,
                                                                        "BM25 parameters k1, b, and avgdl must be set");
            }
...
class SparseDspIndexNodeCC : public SparseDspIndexNode<T> {
 ...
 public:
    explicit SparseDspIndexNodeCC(const int32_t& version, const Object& object)
        : SparseDspIndexNode<T>(version, object) {
    }
    ...
    AnnIterator(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset,
        ...
        return SparseDspIndexNode<T>::AnnIterator(dataset, std::move(cfg), bitset, use_knowhere_search_pool,
                                                  op_context);
    }
    ...
    int64_t
    Dim() const override {
        ReadPermission p(*this);
        return SparseDspIndexNode<T>::Dim();
    }
    int64_t
    Size() const override {
        ReadPermission p(*this);
        return SparseDspIndexNode<T>::Size();
    }
    int64_t
    Count() const override {
        ReadPermission p(*this);
        return SparseDspIndexNode<T>::Count();
    }
