...
///
/// Accumulates (vector id, score) pairs during a retrieval algorithm.
...
struct topk_queue {
    ...
    explicit topk_queue(std::size_t k, float initial_threshold = 0.0F)
    ...
    topk_queue&
    operator=(topk_queue const&) = default;
    topk_queue&
    operator=(topk_queue&&) noexcept = default;
    ~topk_queue() = default;
    ...
    auto
    insert(float score, uint32_t vec_id = 0) -> bool {
        if (not would_enter(score)) [[unlikely]] {
            return false;
        }
        m_q.emplace_back(score, vec_id);
    ...
    would_enter(float score) const {
        return score > m_effective_threshold;
    }
    ...
    finalize() {
        ...
        size_t size = std::lower_bound(m_q.begin(), m_q.end(), 0,
    ...
    [[nodiscard]] std::vector<entry_type> const&
    topk() const noexcept {
        return m_q;
    }
    ...
    [[nodiscard]] auto
    capacity() const noexcept -> std::size_t {
        return m_k;
    }
 ...
 private:
    ...
    static void
    sift_down(entry_iterator_type first, entry_iterator_type last) {
        auto cmp = [first](std::size_t lhs, std::size_t rhs) { return (first + lhs)->first > (first + rhs)->first; };
        auto swap = [first](std::size_t lhs, std::size_t rhs) { return std::iter_swap(first + lhs, first + rhs); };
