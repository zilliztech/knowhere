...
class DspIndexBase {
 public:
    ...
    Search(const SparseRow<T>& query, size_t k, float* distances, label_t* labels, const BitsetView& bitset,
           const DocValueComputer<T>& computer, DspSearchParams& params) const = 0;
    ...
    n_cols() const = 0;
    virtual void
    SetBM25Params(float k1, float b, float avgdl) = 0;
};
 ...
 private:
    ...
    collect_result(HeapType& heap, float* distances, label_t* labels) const {
        ...
        for (auto i = cnt - 1; i >= 0; --i) {
            const auto& entry = heap.Results().front();
            labels[i] = entry.second;
            distances[i] = entry.first;
            heap.Pop();
        }
    }
    ...
    // ========================================================================
    void
    refine_and_collect(const SparseRow<DType>& query, DspHeap& inacc_heap, size_t k, float* distances, label_t* labels,
                       const DocValueComputer<float>& computer) const {
        DspHeap heap(k);
        while (inacc_heap.Size() > 0) {
            table_t doc_id = inacc_heap.Pop()->second;
    ...
    void
    search_dsp(const std::vector<std::pair<size_t, DType>>& q_vec, DspHeap& heap, size_t heap_capacity,
