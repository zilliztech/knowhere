...
str_to_svs_storage_kind(const std::string& s) {
    if (s == "fp32") return faiss::SVS_FP32;
    if (s == "fp16") return faiss::SVS_FP16;
    if (s == "sqi8") return faiss::SVS_SQI8;
    if (s == "lvq4x0") return faiss::SVS_LVQ4x0;
    if (s == "lvq4x4") return faiss::SVS_LVQ4x4;
    if (s == "lvq4x8") return faiss::SVS_LVQ4x8;
    if (s == "leanvec4x4") return faiss::SVS_LeanVec4x4;
    if (s == "leanvec4x8") return faiss::SVS_LeanVec4x8;
    if (s == "leanvec8x8") return faiss::SVS_LeanVec8x8;
    return std::nullopt;
}
...
class SvsVamanaIndexNode : public IndexNode {
 public:
            ...
            for (int i = 0; i < nq; ++i) {
                futs.emplace_back(search_pool_->push([&, index = i] {
                    ...
                    if (range_filter != defaultRangeFilter) {
                        FilterRangeSearchResultForOneNq(result_dist_array[index], result_id_array[index], is_similarity, radius,
                                                        range_filter);
                    }
                }));
            }
    ...
    DeserializeFromFile(const std::string& filename, std::shared_ptr<Config> cfg) override {
        ...
        try {
            auto index = std::unique_ptr<faiss::Index>(faiss::read_index(filename.data(), io_flags));
            if (!dynamic_cast<faiss::IndexSVSVamana*>(index.get())) {
                return Status::invalid_binary_set;
            }
            index_.reset(static_cast<faiss::IndexSVSVamana*>(index.release()));
 ...
 public:
    ...
    Train(const DataSetPtr dataset, std::shared_ptr<Config> cfg, bool use_knowhere_build_pool) override {
        ...
        try {
            ...
            auto idx = std::make_unique<faiss::IndexSVSVamanaLeanVec>(dataset->GetDim(),
                                                                      lv_cfg.svs_graph_max_degree.value(),
                                                                      metric.value(), leanvec_dim, storage.value());
...
KNOWHERE_MOCK_REGISTER_DENSE_FLOAT_ALL_GLOBAL(SVS_VAMANA, SvsVamanaIndexNode, knowhere::feature::KNN)

KNOWHERE_MOCK_REGISTER_DENSE_FLOAT_ALL_GLOBAL(SVS_VAMANA_LVQ, SvsVamanaLvqIndexNode, knowhere::feature::KNN)

KNOWHERE_MOCK_REGISTER_DENSE_FLOAT_ALL_GLOBAL(SVS_VAMANA_LEANVEC, SvsVamanaLeanVecIndexNode, knowhere::feature::KNN)
