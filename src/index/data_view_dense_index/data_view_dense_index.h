...
class DataViewIndexBase {
 public:
    ...
    virtual void
    Add(idx_t n, const void* x, const float* norms_) = 0;

    virtual void
    Search(const idx_t n, const void* x, const idx_t k, float* distances, idx_t* labels,
     ...
     */
    virtual void
    SearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids, const idx_t k,
                  float* out_dist, idx_t* out_ids) const = 0;
    ...
    RangeSearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids, const float radius,
                       const float range_filter) const = 0;
...
class DataViewIndexFlat : public DataViewIndexBase {
 public:
    ...
    void
    SearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids, const idx_t k,
                  float* out_dist, idx_t* out_ids) const override;
    ...
    RangeSearchResult
    RangeSearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids, const float radius,
                       const float range_filter) const override;

    void
    ComputeDistanceSubset(const void* x, const idx_t sub_y_n, float* x_y_distances,
                          const idx_t* x_y_labels) const override;

    float
    GetDataNorm(idx_t id) const {
        assert(id < ntotal_);
        if ((idx_t)norms_.size() < id) {  // maybe cosine is false, get norm in place
...
void
exhaustive_search_one_query_impl(const std::unique_ptr<faiss::DistanceComputer>& computer, size_t ny,
                                 SingleResultHandler& resi, const SelectorHelper& selector,
                                 const bool is_cosine = false, const float* norms_ = nullptr) {
    if (is_cosine && norms_ == nullptr) {
        throw std::runtime_error("Please provide norms if is_cosine == true.");
    }
    auto filter = [&selector](const size_t j) { return selector.is_member(j); };
    if (is_cosine) {
        auto apply = [&resi, &norms_](const float dis, const idx_t j) {
            auto dist_with_norm = dis / (norms_[j]);
            resi.add_result(dist_with_norm, j);
        };
...
void
DataViewIndexFlat::Search(const idx_t n, const void* x, const idx_t k, float* distances, idx_t* labels,
        ...
        if (metric_type_ == metric::L2) {
            ...
            for (auto i = 0; i < n; i++) {
                futs.emplace_back(search_pool->push([&] {
                    ...
                    resi.begin(i);
                    if (bitset.empty()) {
                        exhaustive_search_one_query_impl(computer, n, resi, faiss::IDSelectorAll(), base_norms.get());
                    } else {
                        exhaustive_search_one_query_impl(computer, n, resi, BitsetViewIDSelector(bitset),
                                                         base_norms.get());
                    }
                    resi.end();
                }));
...
void
DataViewIndexFlat::SearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids,
                                 const idx_t k, float* out_dist, idx_t* out_ids) const {
    const auto& search_pool = ThreadPool::GetGlobalSearchThreadPool();
    std::vector<folly::Future<folly::Unit>> futs;
    futs.reserve(n);
    for (auto i = 0; i < n; i++) {
...
RangeSearchResult
DataViewIndexFlat::RangeSearch(const idx_t n, const void* x, const float radius, const float range_filter,
                               const BitsetView& bitset) const {
    // todo: need more test to check
    ...
    if (metric_type_ == metric::L2) {
        for (auto i = 0; i < n; i++) {
            futs.emplace_back(search_pool->push([&, i = i] {
                ...
                reshi.begin(i);
                if (bitset.empty()) {
                    exhaustive_search_one_query_impl(computer, n, reshi, faiss::IDSelectorAll(), base_norms.get());
                } else {
                    exhaustive_search_one_query_impl(computer, n, reshi, BitsetViewIDSelector(bitset),
                                                     base_norms.get());
                }
                reshi.end();
...
RangeSearchResult
DataViewIndexFlat::RangeSearchWithIds(const idx_t n, const void* x, const idx_t* ids_num_lims, const idx_t* ids,
                                      const float radius, const float range_filter) const {
    std::vector<std::vector<float>> result_dist_array(n);
    std::vector<std::vector<idx_t>> result_id_array(n);
    auto is_ip = metric_type_ == metric::IP;
    const auto& search_pool = ThreadPool::GetGlobalSearchThreadPool();
...
void
DataViewIndexFlat::ComputeDistanceSubset(const void* x, const idx_t sub_y_n, float* x_y_distances,
                                         const idx_t* x_y_labels) const {
    auto computer = SelectDataViewComputer(this);
