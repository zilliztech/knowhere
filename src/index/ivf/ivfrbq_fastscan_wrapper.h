...
struct IndexIVFRaBitQFastScanWrapper : faiss::Index {
    ...
    /// Validate request-level preconditions. Call once per request, not per
    /// query, because the FastScan restrictions do not depend on the query
    /// contents.
    static void
    validate_search(const faiss::IDSelector* sel);
