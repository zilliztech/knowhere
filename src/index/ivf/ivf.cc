...
class IvfIndexNode : public IndexNode {
 ...
 private:
    ...
    static constexpr bool
    IsQuantized() {
        return std::is_same_v<IndexType, IndexIVFPQWrapper> ||
...
IvfIndexNode<DataType, IndexType>::TrainInternal(const DataSetPtr dataset, std::shared_ptr<Config> cfg) {
    ...
    if constexpr (std::is_same<IndexIVFPQWrapper, IndexType>::value) {
        ...
        auto result = IndexIvfFactory::create_for_pq(qzr.get(), dim, nlist, nbits, ivf_pq_cfg, data_format, metric.value());
        if (!result.has_value()) {
            return result.error();
        }
        ...
        for (int i = 0; i < rows; ++i) {
            futs.emplace_back(search_pool_->push([&, index = i] {
                ...
                if constexpr (std::is_same<IndexType, faiss::IndexBinaryIVF>::value) {
                    ...
                    const bool whether_to_enable_refine = ivf_cfg.refine_k.has_value();
                    ...
                    if (use_refine && whether_to_enable_refine) {
                        ...
                        faiss::IndexRefineSearchParameters refine_search_params;
                        refine_search_params.sel = id_selector;
                        refine_search_params.k_factor = ivf_cfg.refine_k.value_or(1);
                        refine_search_params.base_index_params = &ivf_search_params;
