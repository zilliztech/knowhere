...
template <typename IndexIVFType>
void
IndexIVFWrapper<IndexIVFType>::search(faiss::idx_t n, const float* x, faiss::idx_t k, float* distances, faiss::idx_t* labels,
...
template <typename IndexIVFType>
void
IndexIVFWrapper<IndexIVFType>::range_search(faiss::idx_t n, const float* x, float radius, faiss::RangeSearchResult* result,
                                const faiss::SearchParameters* params) const {
...
IndexIVFWrapper<IndexIVFType>::getIteratorNextBatch(faiss::IVFIteratorWorkspace* workspace, size_t current_backup_count) const {
...
template
typename std::enable_if<std::is_same_v<faiss::IndexIVFScalarQuantizer, faiss::IndexIVFScalarQuantizer>, std::unique_ptr<faiss::IVFIteratorWorkspace>>::type
IndexIVFWrapper<faiss::IndexIVFScalarQuantizer>::getIteratorWorkspace<faiss::IndexIVFScalarQuantizer>(
    const float* query_data, 
    const faiss::IVFSearchParameters* ivfsearchParams
) const;
...
IndexIVFWrapper<faiss::IndexIVFScalarQuantizer>::getIteratorNextBatch<faiss::IndexIVFScalarQuantizer>(
    faiss::IVFIteratorWorkspace* workspace, 
    size_t current_backup_count
) const;

expected<std::unique_ptr<IndexIVFPQWrapper>>
IndexIvfFactory::create_for_pq(faiss::IndexFlat* qzr_raw_ptr,
    ...
    if (ivf_pq_cfg.refine.value_or(false) && ivf_pq_cfg.refine_type.has_value()) {
        ...
        if (!final_index_cnd.has_value()) {
            return expected<std::unique_ptr<IndexIVFPQWrapper>>::Err(Status::invalid_args,
...
IndexIvfFactory::create_for_sq(faiss::IndexFlat* qzr_raw_ptr,
    ...
    if (ivf_sq_cfg.refine.value_or(false) && ivf_sq_cfg.refine_type.has_value()) {
        ...
        if (!final_index_cnd.has_value()) {
            return expected<std::unique_ptr<IndexIVFSQWrapper>>::Err(Status::invalid_args,
                                                                     "Invalid refine parameters");
        }
