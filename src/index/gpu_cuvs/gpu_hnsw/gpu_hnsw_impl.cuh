...
search(cudaStream_t stream, const search_params& params, const gpu_hnsw_index& idx, int num_queries, int k) {
    ...
    if (num_upper_layers > 0) {
        // Use pre-built upper layer pointers (uploaded at build time).
        ...
        kernel::upper_layer_search_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
            sc.d_queries, idx.d_dataset, d_layer_ptrs, sc.d_entry_points, idx.entry_point, num_queries, dim,
    ...
    // Zero the visited bitmap (pre-allocated in scratch, just memset)
    ...
    kernel::layer0_beam_search_kernel<<<num_queries, block_size, smem_size, stream>>>(
        sc.d_queries, idx.d_dataset, idx.d_layer0_graph, sc.d_entry_points, sc.d_visited_bitmaps, sc.d_neighbors,
        sc.d_distances, num_queries, N_int, dim, idx.max_degree0, k, ef, sw, max_iter, idx.use_ip);
    GPU_HNSW_CUDA_CHECK(cudaGetLastError());
}
