...
convert_rows_to_fp32(const void* const __restrict src_in, float* const __restrict dst,
                     const DataFormatEnum src_data_format, const uint32_t* offsets, const size_t nrows,
                     const size_t dim) {
...
// there are chances that each partition split by scalar distribution is too small that we could not evn train pq on it
 ...
 protected:
    ...
    next_batch(std::function<void(const std::vector<DistId>&)> batch_handler) override {
        if (workspace.bitset.empty()) {
            ...
            filter_type sel(workspace.bitset, 0, labels == nullptr ? nullptr : labels->data());
...
class BaseFaissRegularIndexHNSWNode : public BaseFaissRegularIndexNode {
 public:
    ...
    GetVectorByIds(const DataSetPtr dataset) const override {
        ...
        auto get_vector = [&](int64_t id, float* result) {
            if (indexes.size() == 1) {
    ...
    Search(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset) const override {
        ...
        // set up a selector
        BitsetViewIDSelector bw_idselector(bitset, 0, labels.empty() ? nullptr : labels[index_id].get()->data());
        ...
        try {
            ...
            for (int64_t i = 0; i < rows; ++i) {
                futs.emplace_back(search_pool->push([&, idx = i, is_refined = is_refined,
                    ...
                    if (!labels.empty()) {
                        for (auto i = 0; i < k; ++i) {
                            local_ids[i] = local_ids[i] < 0 ? local_ids[i] : labels[index_id]->operator[](local_ids[i]);
                        }
                    }
                }));
            }
            ...
            futs.emplace_back(
                search_pool->push([&, idx = i, is_refined = is_refined, index_wrapper_ptr = index_wrapper_ptr] {
                    ...
                    if (labels.empty()) {
                        ...
                        for (size_t j = 0; j < elem_cnt; j++) {
                            result_dist_array[idx][j] = res.distances[j];
                            result_id_array[idx][j] = labels[index_id]->operator[](res.labels[j]);
                        }
                    }
 ...
 protected:
    ...
    AddInternal(const DataSetPtr dataset, const Config&) override {
        ...
        const std::vector<std::vector<uint32_t>>& scalar_info =
            dataset->Get<std::vector<std::vector<uint32_t>>>(meta::SCALAR_INFO);
        ...
        LOG_KNOWHERE_INFO_ << "add to hnsw index with scalar info";
        try {
            for (auto i = 0; i < tmp_combined_scalar_ids.size(); ++i) {
                for (auto j = 0; j < tmp_combined_scalar_ids[i].size(); ++j) {
                    ...
                    auto status = add_partial_dataset_to_index(indexes[i].get(), dataset, data_format, scalar_info[id]);
...
class BaseFaissRegularIndexHNSWFlatNode : public BaseFaissRegularIndexHNSWNode {
 ...
 protected:
    ...
    TrainInternal(const DataSetPtr dataset, const Config& cfg) override {
        ...
        auto create_index = [&](const float* data, const int i, const int64_t rows) {
            if (is_cosine) {
                if (data_format == DataFormatEnum::fp32) {
        ...
        if (scalar_info.size() <= 1 || tmp_combined_scalar_ids.size() <= 1) {
            return create_index((const float*)(data), 0, rows);
        }
        ...
        for (auto i = 0; i < tmp_combined_scalar_ids.size(); ++i) {
            ...
            Status s = create_index((const float*)(tmp_data.get()), i, partition_size);
            if (s != Status::success) {
                return s;
            }
        }
 ...
 protected:
    ...
    TrainInternal(const DataSetPtr dataset, const Config& cfg) override {
        ...
        if (scalar_info.size() <= 1 || tmp_combined_scalar_ids.size() <= 1) {
            ...
            return create_index(reinterpret_cast<const float*>(float_ds_ptr->GetTensor()), 0, rows);
        }
        LOG_KNOWHERE_INFO_ << "train hnsw index with scalar info";
 ...
 protected:
    ...
    TrainInternal(const DataSetPtr dataset, const Config& cfg) override {
        ...
        if (scalar_info.size() <= 1 || tmp_combined_scalar_ids.size() <= 1) {
            ...
            return create_index((const float*)(float_ds_ptr->GetTensor()), 0, rows);
        }
