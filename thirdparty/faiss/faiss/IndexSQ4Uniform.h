 ...
 * Assumes both query and base vectors are normalized (||q||=1, ||b||=1).
...
struct SQ4UniformCosineDistanceComputer : DistanceComputer {
    ...
    /// Set query vector (assumes already normalized by caller)
 ...
 * metric. Assumes vectors are already normalized by caller (faiss_hnsw.cc).
 * Implements HasInverseL2Norms to return all 1.0 (normalized vectors).
 */
struct IndexScalarQuantizer4bitUniformCosine : IndexScalarQuantizer,
    ...
    mutable std::vector<float> inverse_norms_cache;

    IndexScalarQuantizer4bitUniformCosine(int d);
    IndexScalarQuantizer4bitUniformCosine();

    /// Add normalized vectors (assumes caller has normalized them)
