...
struct MmappedFileMappingOwner::PImpl {
    ...
    PImpl(FILE* f) {
        ...
        void* address = mmap(nullptr, filesize, PROT_READ, MAP_SHARED, fileno(f), 0);
        FAISS_THROW_IF_NOT_FMT(
...
MmappedFileMappingOwner::MmappedFileMappingOwner(const std::string& filename) {
    auto fd = std::unique_ptr<FILE, decltype(&fclose)>(fopen(filename.c_str(), "r"), &fclose);
    FAISS_THROW_IF_NOT_FMT(
            fd.get(), "could not open %s for reading: %s", filename.c_str(), strerror(errno));
...
MappedFileIOReader::MappedFileIOReader(const std::shared_ptr<MmappedFileMappingOwner>& owner) :
    mmap_owner(owner) {}
