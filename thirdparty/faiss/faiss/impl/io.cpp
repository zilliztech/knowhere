...
size_t BlockFileIOWriter::write(const char* ptr, size_t bytes_size) {
    if (block_buf_ofs + bytes_size <= block_size) {
        ...
        while (cur_pos < bytes_size) { 
            size_t copy_size = std::min(block_size - block_buf_ofs, bytes_size - cur_pos);
...
size_t BlockFileIOWriter::operator()(const void* ptr, size_t size, size_t nitems) {
    return write((const char*)ptr, size * nitems);
}

size_t BlockFileIOWriter::write_header(const char* ptr, size_t bytes) {
    FAISS_THROW_IF_MSG(bytes > block_size, "header size should not larger than a block size");
    fseek(f, 0, SEEK_SET);
