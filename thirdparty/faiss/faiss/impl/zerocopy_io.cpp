...
size_t
ZeroCopyIOReader::getDataView(void** ptr, size_t size, size_t nitems) {
    if (size == 0) {
        return nitems;
    }
...
void
ZeroCopyIOReader::reset() {
    rp_ = 0;
}

size_t
ZeroCopyIOReader::operator()(void* ptr, size_t size, size_t nitems) {
    if (rp_ >= total_) {
        return 0;
    }
...
int
ZeroCopyIOReader::filedescriptor() {
