...
void RaBitQuantizer::compute_codes(
    const float* x, 
    uint8_t* codes, 
    size_t n
) const {
...
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < n; i++) {
        ...
        for (size_t j = 0; j < d; j++) {
            const float or_minus_c = x[i * d + j] - ((centroid_in == nullptr) ? 0 : centroid_in[j]);
...
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < n; i++) {
        ...
        for (size_t j = 0; j < d; j++) {
            ...
            x[i * d + j] =
                    (bit - 0.5f) * fac->factor_2 * 2 * inv_d_sqrt + ((centroid_in == nullptr) ? 0 : centroid_in[j]);
        }
    }
}
...
float RaBitDistanceComputerQ::distance_to_code_L2(
        const uint8_t* code) const {
    // split the code into parts
    const uint8_t* binary_data = code;
    const FactorsData* fac =
    ...
    //     // extract i-th bit
    //     const uint8_t masker = (1 << (i % 8));
    //     const uint8_t bit = ((binary_data[i / 8] & masker) == masker) ? 1 : 0;
    //
    //     // accumulate dp
...
float RaBitDistanceComputerQ::distance_to_code_IP(
        const uint8_t* code) const {
    // split the code into parts
    const FactorsData* fac =
            reinterpret_cast<const FactorsData*>(code + (d + 7) / 8);
...
void RaBitDistanceComputerQ::set_query(const float* x) {
    ...
    for (size_t idim = 0; idim < d; idim++) {
        for (size_t iv = 0; iv < qb; iv++) {
            ...
            rearranged_rotated_qq[iv * offset + idim / 8] |= bit ? (1 << (idim % 8)) : 0;
        }
    }    
