...
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < static_cast<int64_t>(n); i++) {
        ...
        const size_t base_code_size = dense_layout
                ? (d * nb_bits + 7) / 8
                : (d + 7) / 8;
            ...
            if (dense_layout) {
                for (size_t j = 0; j < d; j++) {
                    const uint16_t sign = residual[j] > 0.0f
                            ...
                            : 0;
                    const uint16_t value = sign | static_cast<uint16_t>(
            ...
            const float bit = dense_layout
                    ? ((rabitq_utils::extract_code_inline(
                        ...
                        (1u << ex_bits)) != 0
                               ? 1.0f
                               : 0.0f)
                    : (((binary_data[j / 8] & masker) == masker) ? 1.0f
                                                                  : 0.0f);
...
void distance_to_code_full_batch_4_impl(
    ...
    const size_t code_size_base = dense_layout
            ? (d * nb_bits + 7) / 8
            : (d + 7) / 8;
