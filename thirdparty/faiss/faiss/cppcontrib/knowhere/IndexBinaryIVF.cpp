...
void IndexBinaryIVF::search(
    ...
    quantizer->search(
            ...
            idx.get(),
            quantizer_params);
    ::faiss::indexIVF_stats.quantization_time += getmillisecs() - t0;
    ...
    search_preassigned(
            ...
            false,
            params,
            &::faiss::indexIVF_stats);
    ::faiss::indexIVF_stats.search_time += getmillisecs() - t0;
}
...
void IndexBinaryIVF::search_and_reconstruct(
    ...
    quantizer->search(n, x, nprobe_2, coarse_dis.get(), idx.get(), quantizer_params);
...
void IndexBinaryIVF::replace_invlists(::faiss::InvertedLists* il, bool own) {
...
struct IVFBinaryScannerL2 : BinaryInvertedListScanner {
    ...
    ::faiss::InvertedListScannerStats scan_codes(
            size_t n,
        ...
        ::faiss::InvertedListScannerStats stats;
    ...
    ::faiss::InvertedListScannerStats scan_codes_range(
            size_t n,
            const uint8_t* __restrict codes,
            const idx_t* __restrict ids,
            float radius,
...
void IndexBinaryIVF::search_preassigned(
    ...
    if (metric_type == METRIC_Jaccard) {
        ...
        if (per_invlist_search) {
            ...
            dispatch_HammingComputer(
                    code_size, r, this, n, x, cidx, k, dis, idx, params, stats);
...
void IndexBinaryIVF::range_search(
        ...
        const uint8_t* __restrict x,
        float radius,
        faiss::RangeSearchResult* __restrict res,
        const SearchParameters* params_in) const {
    const IVFSearchParameters* params = nullptr;
    const SearchParameters* quantizer_params = nullptr;
    if (params_in) {
    ...
    double t0 = getmillisecs();
    quantizer->search(
            n, x, nprobe_2, coarse_dis.get(), idx.get(), quantizer_params);
    ::faiss::indexIVF_stats.quantization_time += getmillisecs() - t0;
...
void IndexBinaryIVF::range_search_preassigned(
        ...
        const IVFSearchParameters* params,
        IndexIVFStats* stats) const {
    idx_t nprobe_2 = params ? params->nprobe : this->nprobe;
    nprobe_2 = std::min((idx_t)nlist, nprobe_2);
    FAISS_THROW_IF_NOT(nprobe_2 > 0);
