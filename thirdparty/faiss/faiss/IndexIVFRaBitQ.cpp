...
void IndexIVFRaBitQ::train_encoder(idx_t n, const float* x, const idx_t* assign) {
    rabitq.train(n, x);
}
...
#pragma omp for
        for (idx_t i = 0; i < n; i++) {
            ...
            if (list_no >= 0) {
                ...
                rabitq.compute_codes_core(xi, code + coarse_size, 1, centroid.data());                
...
#pragma omp parallel
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (list_no >= 0 && list_no % nt == rank) {
                ...
                // both by_residual and !by_residual lead to the same code
                quantizer->reconstruct(list_no, centroid.data());
                rabitq.compute_codes_core(xi, one_code.data(), 1, centroid.data()); 
...
struct RaBitInvertedListScanner : InvertedListScanner {
    ...
    RaBitInvertedListScanner(
        ...
        uint8_t qb_in = 0
    ) : InvertedListScanner(store_pairs, sel), ivf_rabitq{ivf_rabitq_in}, qb{qb_in} {
    ...
    void set_list(idx_t list_no, float coarse_dis) override {
        ...
        ivf_rabitq.quantizer->reconstruct(list_no, reconstructed_centroid.data());
    ...
    void internal_try_setup_dc() {
        if (!query_vector.empty() && !reconstructed_centroid.empty()) {
            ...
            dc.reset(
                ivf_rabitq.rabitq.get_distance_computer(
                    qb, 
                    ivf_rabitq.metric_type, 
...
InvertedListScanner* IndexIVFRaBitQ::get_InvertedListScanner_2(
        ...
        const IVFSearchParameters* search_params_in
) const {
    uint8_t used_qb = qb;
    if (search_params_in != nullptr) {
        const auto* search_params = dynamic_cast<const IVFRaBitQSearchParameters*>(search_params_in);
...
void IndexIVFRaBitQ::reconstruct_from_offset(int64_t list_no, int64_t offset, float* recons) const {
...
float IVFRaBitDistanceComputer::operator()(idx_t i) {
    ...
    std::unique_ptr<RaBitQuantizer::RaBitDistanceComputer> dc(
        parent->rabitq.get_distance_computer(parent->qb, parent->metric_type, centroid.data())
    );
    dc->set_query(q);
