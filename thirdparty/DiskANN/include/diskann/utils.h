...
namespace diskann {
    ...
    inline void get_bin_metadata_impl(std::basic_istream<char>& reader,
                                      size_t& nrows, size_t& ncols, size_t offset = 0) {
    ...
    inline void load_bin(const std::string& bin_file, std::unique_ptr<T[]>& data, size_t& npts,
        ...
        try {
            LOG_KNOWHERE_INFO_ << "Opening bin file " << bin_file.c_str() << "... ";
    ...
    inline void open_file_to_write(std::ofstream& writer,
        ...
        LOG_KNOWHERE_INFO_ << "Open bin: " << filename.c_str();
    }
    ...
    inline uint64_t save_bin(const std::string& filename, T* data, size_t npts,
        ...
        LOG_KNOWHERE_INFO_ << "Writing bin: " << filename.c_str();
        int npts_i32 = (int) npts, ndims_i32 = (int) ndims;
        writer.write((char*) &npts_i32, sizeof (int));
        writer.write((char*) &ndims_i32, sizeof (int));
        LOG_KNOWHERE_INFO_ << "bin: #pts = " << npts << ", #dims = " << ndims
                            << ", size = "
    ...
    inline std::string get_pq_compressed_rearranged_filename(const std::string& prefix) {
        return prefix + "_pq_compressed_rearranged.bin";
    }
