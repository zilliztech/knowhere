...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    HierarchicalNSW(SpaceInterface<dist_t>* s, size_t max_elements, size_t M = 16, size_t ef_construction = 200,
        ...
        space_ = s;
        sq_enabled_ = use_sq;
        sq_refine_enabled_ = use_sq_refine;
        ...
        if (!sq_enabled_ || sq_refine_enabled_) {
    ...
    bool sq_enabled_ = false;
    bool sq_refine_enabled_ = false;
    float alpha_ = 0.0f;
    ...
    getNeighboursWithinRadius(NeighborSet& top_candidates, const void* data_point, float radius,
    ...
    void
    loadIndex(const std::string& location, const knowhere::Config& config, bool use_sq, size_t max_elements_i = 0) {
        ...
        if (use_sq) {
            sq_enabled_ = true;
            readBinaryPOD(input, sq_refine_enabled_);
    ...
    saveIndex(knowhere::MemoryIOWriter& output) {
        ...
        if (sq_enabled_) {
            writeBinaryPOD(output, sq_refine_enabled_);
    ...
    void
    loadIndex(knowhere::MemoryIOReader& input, bool use_sq, size_t max_elements_i = 0) {
        using knowhere::readBinaryPOD;
