...
// ============================================================================
// AVX2 SIMD + Prefetching Implementation
// ============================================================================

#ifdef __AVX2__
template <typename QType, typename Computer>
std::vector<float> compute_all_distances_avx2(
    const std::vector<std::pair<size_t, QType>>& q_vec,
    const std::vector<std::span<const table_id_t>>& inverted_index_ids_spans,
    const std::vector<std::span<const float>>& inverted_index_vals_spans,
    size_t n_rows,
    ...
    // AVX2 only for IP metric in this benchmark
    if (metric_type != SparseMetricType::METRIC_IP) {
        return compute_all_distances_prefetch<QType>(q_vec, inverted_index_ids_spans,
                                                      inverted_index_vals_spans, n_rows,
                                                      metric_type, computer, row_sums_spans);
    }
...
// ============================================================================
// AVX512 SIMD + Prefetching Implementation
// ============================================================================

#ifdef __AVX512F__
template <typename QType, typename Computer>
std::vector<float> compute_all_distances_avx512(
    const std::vector<std::pair<size_t, QType>>& q_vec,
    const std::vector<std::span<const table_id_t>>& inverted_index_ids_spans,
    const std::vector<std::span<const float>>& inverted_index_vals_spans,
    size_t n_rows,
    ...
    // AVX512 only for IP metric in this benchmark
    if (metric_type != SparseMetricType::METRIC_IP) {
        return compute_all_distances_prefetch<QType>(q_vec, inverted_index_ids_spans,
                                                      inverted_index_vals_spans, n_rows,
                                                      metric_type, computer, row_sums_spans);
...
int main(int argc, char** argv) {
    ...
    for (auto metric_type : {SparseMetricType::METRIC_IP, SparseMetricType::METRIC_BM25}) {
        ...
        results.push_back(run_benchmark("Prefetching Only", index_data, queries, prefetch_func));

        // AVX2 + Prefetching
#ifdef __AVX2__
        if (caps.has_avx2) {
            auto avx2_func = [&](const std::vector<std::pair<size_t, float>>& q) {
                return compute_all_distances_avx2<float>(
                    q, id_spans, val_spans, index_data.n_rows,
                    metric_type, identity, row_sums_span
                );
            };
            results.push_back(run_benchmark("AVX2 + Prefetch", index_data, queries, avx2_func));
        }
#endif
...
#ifdef __AVX512F__
        if (caps.has_avx512f) {
            auto avx512_func = [&](const std::vector<std::pair<size_t, float>>& q) {
                return compute_all_distances_avx512<float>(
                    q, id_spans, val_spans, index_data.n_rows,
                    metric_type, identity, row_sums_span
                );
            };
            results.push_back(run_benchmark("AVX512 + Prefetch", index_data, queries, avx512_func));
        }
#endif
    ...
    std::cout << "║  • SIMD + Prefetching combines parallelism with latency   ║\n";
    std::cout << "║    hiding for maximum performance                          ║\n";
    std::cout << "║  • AVX512 has true scatter: 16 results in 1 instruction    ║\n";
    std::cout << "║  • BM25 uses prefetching (SIMD only for IP metric)        ║\n";
